...
Span *PageCache::NewSpan(size_t npage)
{
   ...
   std::lock_guard<std::recursive_mutex> lock(_mtx);
    ...
    if (npage >= NPAGES)
    {
        ...
        // std::cout<<" big memory"<<span->_pageid<<" "<<span->_npage<<std::endl;
...
Span *PageCache::_NewSpan(size_t npage)
{
    ...
    for (size_t i = npage + 1; i < NPAGES; i++) 
    {
        if (!_pagelist[i].Empty())
        {
            ...
            //std::cout<<split->_pageid<<std::endl;
            // 将新分割出来的页都映射到新的span上
...
Span *PageCache::MapObjectToSpan(void *obj)
{
    ...
    // return it->second;
    Span *span = _id_span_map.find(pageid);
    if (span != nullptr)
    {
        return span;
    }
...
void PageCache::RelaseToPageCache(Span *span)
{
   ...
   std::lock_guard<std::recursive_mutex> lock(_mtx);
    ...
    if (span->_npage >= NPAGES)
    {
        ...
        SystemFree(ptr, (span->_npage << PAGE_SHIFT));
        delete span;
    ...
    while (1)
    {
        ...
        Span *prevspan = _id_span_map.find(preid);
        if (prevspan == nullptr)
        {
            break;
        }
        ...
        // prevspan = _id_span_map.find(span->_pageid - 1);
    }
    ...
    while (1)
    {
        ...
        Span *nextspan = _id_span_map.find(nextId);
        if (nextspan == nullptr)
        {
